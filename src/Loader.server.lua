...
-- (Copied from Hoarcekat (it already had the comment in it...))

local PLUGIN_SOURCE_NAME = "RobloxTrackBuilderPlugin"
...
if useDevSource then
	...
	currentRoot = source
		...
		warn(PLUGIN_SOURCE_NAME .. "Development source is not present, running using built-in source.")
	end
end
...
function PluginFacade:_watch(instance)
	...
	local connection2 = instance.ChildAdded:Connect(function(childInstance)
		self:_watch(childInstance)
	end)
